...
def plot_wine_neural_network_summary(results_nn, save_dir):
    """
    Plot summary for Neural Network (MLP) on the Wine dataset.

    results_nn: results["neural_network"] from wine.py
    """
    ...
    plt.title("Wine Dataset – Neural Network: Accuracy vs Train/Test Split", fontsize=12)
    plt.legend(loc="best")
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, "wine_nn_accuracy.png"), dpi=150, bbox_inches="tight")
    plt.close()
    ...
    if best_model is not None:
        ...
        plt.title("Wine Dataset – Neural Network Confusion Matrix (best model)", fontsize=12)
        plt.colorbar(im)
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(save_dir, "wine_nn_confusion_best_model.png"), dpi=150, bbox_inches="tight")
        plt.close()

    # Text report
    report_path = os.path.join(save_dir, "wine_neural_network_report.txt")
    with open(report_path, "w") as f:
        f.write("Wine Dataset – Neural Network (MLP) Summary Report\n")
        f.write("=" * 70 + "\n\n")
    ...
    print(f"[wine_neural_network_plots] Saved plots and report to {save_dir}")
